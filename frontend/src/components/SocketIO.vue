<template>
  <div>
      <div class="btn-group btn-sm" role="group" aria-label="Basic example">
        ...
        <!-- <button v-if="!store.socketIO.pause" type="button" class="btn btn-sm text-light" @click="store.socketIO.pause=!store.socketIO.pause" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Resume log display">
          <i  class="bi bi-play-circle" :class="`${connected ? 'text-success' : 'text-secondary'}`" style="font-size: 1.5rem;" ></i>   
        </button>
        <button v-if="store.socketIO.pause" type="button" class="btn btn-sm text-light" @click="store.socketIO.pause=!store.socketIO.pause" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Pause log display">
          <i class="bi bi-pause-circle" :class="`${connected ? 'text-success' : 'text-secondary'}`" style="font-size: 1.5rem;"></i>        
        </button> -->
